...
from src.modules.schedule.models.models import RegisterSchedule, ScheduleModel
...
class ScheduleService:
    ...
    async def get_schedule_by_doctor(self, doctor_id: int, days: int = 30):
        if not await self.__repository.doctor_exists(doctor_id):
            raise BadRequestException("Doctor not found")

        default_schedule: Sequence[DoctorSchedule] = await self.__repository.get_default_schedule(doctor_id)
        future_schedules: Sequence[Schedule] = await self.__repository.get_future_schedules(doctor_id)
        return await GetDayScheduleDoctor(days, future_schedules).generates_availability(default_schedule)

    async def create_schedule(self, schedule_model: RegisterSchedule) -> ScheduleModel:
        appointment = datetime.combine(schedule_model.schedule_date, time(hour=schedule_model.start_time))
    ...
    async def validate_schedule(self, schedule_model: RegisterSchedule, appointment: datetime) -> None:
        ...
        if await self.__repository.verify_schedule(schedule_model.doctor_id, appointment):
